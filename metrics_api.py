...
# and if it cant find it that means you didnt make one or you didnt copy it or i dont know
...
def _normalise_type(t: str) -> str:
    """Map internal ExperimentMonitor type names to frontend-friendly names."""
    return {
        "cpu_stress":      "cpu",
...
def get_reports_summary(limit: int = Query(default=20, ge=1, le=100)):
    ...
    try:
        docs = (
            db["experiments"]
            .find({"status": {"$in": ["completed", "stopped"]}, "started_at": {"$ne": None}}, {"_id": 0})
    ...
    for exp in experiments:
        raw_type = exp.get("failure_type", "unknown")
        started = exp.get("started_at")
        ended   = exp.get("ended_at")
        if started and isinstance(started, datetime) and started.tzinfo is None:
            started = started.replace(tzinfo=timezone.utc)
        if ended and isinstance(ended, datetime) and ended.tzinfo is None:
        ...
        summaries.append({
            ...
            "completedAt":    ended.isoformat()   if isinstance(ended,   datetime) else ended,
