...
class StateTracker:
    ...
    def handle_state(self, db, current_state: str, active_experiments: list):
        ...
        if self.last_state != "running" and current_state == "running":
            ...
            # figure out the injection type from the active_experiments list
            types = [e.get("type", "unknown") for e in active_experiments]
            failure_type = "+".join(sorted(set(types))) if types else "unknown"

            # build the parameters block from each active experiment
            params = {}
            for exp in active_experiments:
                p = exp.get("params", {})
                if exp.get("type") == "cpu_stress":
                    params["cpu_workers"] = exp.get("intensity")
                elif exp.get("type") == "memory_stress":
                    params["memory_config"] = exp.get("intensity")
                elif exp.get("type") == "network_latency":
                    params["delay"] = p.get("delay")
                elif exp.get("type") == "packet_loss":
        ...
        # unning  to complete or idle : experiment finished 
...
def run():
    ...
    while True:
        ...
        net_data = fetch_json(NETWORK_URL)
