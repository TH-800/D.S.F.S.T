...
def check_stress_ng_running() -> list[dict]:
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline", "create_time", "ppid"]):
        try:
            ...
            # Secondary worker filter: real masters always have multiple args (--cpu N --timeout Ns etc)
            # Worker processes have a stripped cmdline — usually empty or just ["stress-ng"]
            # If cmdline has fewer than 3 items and has no -- flags, it is a worker orphan
            has_flags = any(arg.startswith("--") for arg in cmdline)
            if not has_flags and len(cmdline) < 3:
                continue  # orphaned worker after master exited — skip it
